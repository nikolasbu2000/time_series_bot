...
from tools.stl_decomp import stl_decompose
...
tool = st.selectbox(
    ...
    [
        ...
        "MGARCH (DCC, via R)",
        "STL decomposition (trend/seasonal/residual, Python)",
    ],
)
...
with st.form("params_form", border=True):
    ...
    #STL Decomposition
    elif tool.startswith("STL decomposition"):
        st.caption("STL needs a seasonal period (e.g., 12 monthly, 4 quarterly, 7 daily weekly seasonality).")
        stl_period = st.number_input("Seasonal period", min_value=2, max_value=365, value=12, step=1)
        stl_robust = st.checkbox("robust (less sensitive to outliers)", value=False)
    ...
    # APT (R)
        ...
        # NOTE: apt v4.0 functions do NOT accept digits; keep UI for display only (optional)
        apt_digits = st.number_input("digits (printing; app-side only)", min_value=1, max_value=8, value=3, step=1)
...
if run:
    try:
        ...
        if tool.startswith("ADF"):
            ...
            # Uses the currently selected y_col series (and your per-variable transforms)
            res = stl_decompose(s, period=int(stl_period), robust=bool(stl_robust))
            if not isinstance(res, dict) or res.get("ok") is False:
                st.error("STL failed.")
                st.json(res)
            else:
                st.success("STL computed.")
                ...
                # If you have a real datetime index, you can try parsing for nicer x-axis
                # (safe even if it fails)
                try:
                    df_stl["index_dt"] = pd.to_datetime(df_stl["index"], errors="coerce")
                    xcol = "index_dt" if df_stl["index_dt"].notna().any() else "index"
                except Exception:
                    xcol = "index"

                st.subheader("STL components")
                st.line_chart(df_stl.set_index(xcol)[["observed"]])
                st.line_chart(df_stl.set_index(xcol)[["trend"]])
                st.line_chart(df_stl.set_index(xcol)[["seasonal"]])
                st.line_chart(df_stl.set_index(xcol)[["resid"]])
            ...
            with st.spinner("Running APT via R..."):
                res = apt_via_r(
                    ...
                    # FIX: apt_via_r expects th_range as tuple/None (NOT a float)
                    th_range=(0.0, float(apt_th_range)),
                    # FIX: do NOT pass digits to apt_via_r (apt v4.0 doesn't accept it; wrapper may not expose it)
